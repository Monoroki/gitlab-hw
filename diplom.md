...
Использованные файлы конфигурации: [Diplom](https://github.com/Monoroki/gitlab-hw/tree/main/diplom).
   ...
   - инфраструктура поднимается одним запуском при помощи terraform.
   - програмная среда разворачивается ansible с хостовой машины через proxi которым выступает bastion host
   - для доступа к интернету с приватных подсетей использовался NAT-шлюз
# Шаг1 "Разворачиваем инфраструктуру"

 - с помощью шаблона terraform создаем и заполняем переменными inventory файл ansible
...
# Шаг2 "Разворачиваем необходимые пакеты с помощью playbook-ansible"

 - необходимые конфигурационные файлы создавал с помощью шаблонов ansible подпихивая переменные
 - zabbix сервер поднимал с ansible через docker-compous 
 - пароли в docker-compous передавал с помощью переменных при запуске плейбука
<details>
